...
def _mark_group(tag: str, fiducials: list[Fiducial]) -> etree._Element:
    group = etree.Element(tag, no="1")
    etree.SubElement(group, "markType").text = "BOC"
    etree.SubElement(group, "markName").text = "BOCMARK01"
    ...
    for i in range(SLOTS):
        ...
        if i < len(fiducials):
            ...
            etree.SubElement(mark, "markPosition", x="0", y="0")
    return group
...
def emit_machine(bm: BuildModel) -> etree._Element:
    ...
    cd.append(_mark_group("bocMark", list(bm.cfg.fiducials)))
    cd.append(_mark_group("secondBocMark", []))
    return machine
